...
# Choose whether to use a complex Wide & Deep Classifier or
...
def build_estimator(model_dir, embedding_size=8, hidden_units=None):
  ...
  simple_columns = [group1, group2, group3, group4, group5, group6, nlp_persons]

  if COMPLEX_MODEL:
    # model will use deep and wide column including engineered buckets
    return tf.contrib.learn.DNNLinearCombinedClassifier(
        model_dir=model_dir,
        linear_feature_columns=wide_columns,
        dnn_feature_columns=deep_columns,
        n_classes=6,
        dnn_hidden_units=hidden_units or [100, 100])
  else:
    return tf.contrib.learn.DNNClassifier(
        model_dir=model_dir,
        feature_columns=simple_columns, 
        hidden_units=[50, 50], 
        n_classes=6)
